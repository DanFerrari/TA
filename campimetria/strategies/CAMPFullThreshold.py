...
from TelaResultadoFullThreshold import exibir_resultados
...
class FullThreshold:
    ...
    def main(self):
        ...
        while not all(ponto.status == "=" for ponto in pontos):  # Enquanto nem todos estiverem ativados
            ...
            for ponto in pontos:
                if not ponto.status == "=":  # Apenas testa se ainda não foi ativado
                    ...
                    if testemancha == 100 and teste_de_fixacao:
                        perda_de_fixacao += self.testa_mancha_cega(DadosExame.posicao_mancha_cega)
                        testemancha = 0 
                        testes_realizados += 1
                    if len(tempos) == 5:
                        tempo_resposta = self.media_de_tempo_de_resposta_paciente(tempos)
                        tempos = []
                    print(f"Ponto: ({ponto.x}, {ponto.y}), Atenuacao: {ponto.atenuacao}, Cor: {ponto.cor}")
                    print(f"Ponto definidos: {DadosExame.total_pontos_definidos} Mancha: {testemancha}")
                    
                    
        visualizando = True
        pygame.display.get_surface().fill(Colors.BACKGROUND)
        fonte_label = pygame.font.Font(None, 30)
        perda_de_fixacao = ((perda_de_fixacao / testes_realizados) * 100) if perda_de_fixacao > 0.0 else  0
        texto_label = fonte_label.render(f"Perda de fixacao: {perda_de_fixacao}%", True, (255, 255, 255))
        pygame.display.get_surface().blit(texto_label, (pygame.display.get_surface().get_width() - texto_label.get_width() - 250, pygame.display.get_surface().get_height() - texto_label.get_height() - 150))
        exibir_resultados(pontos)
        while visualizando:   
