...
def desenhar_botao(texto, x, y, largura, altura, selecionado):
    ...
    pygame.draw.rect(pygame.display.get_surface(), cor_atual, (x, y, largura, altura), border_radius=5)
    
    # Se for o botão selecionado, desenha uma borda ao redor
    if selecionado:
        pygame.draw.rect(pygame.display.get_surface(), cor_borda_selecao, (x - 2, y - 2, largura + 4, altura + 4), 2, border_radius=5)
...
def mostrar_alerta(botao_reiniciar_estado, botao_continuar_estado):
    ...
    x, y = (pygame.display.get_surface().get_width() - largura) // 2, (pygame.display.get_surface().get_height() - altura) // 2  # Centro da tela
    
    pygame.draw.rect(pygame.display.get_surface(), cor_alerta, (x, y, largura, altura), border_radius=10)
    
    # Renderiza o texto da notificação
    texto = fonte.render("Mancha cega nao encontrada, deseja continuar mesmo assim ou deseja reiniciar", True, cor_texto)
    ...
    botao_reiniciar = desenhar_botao("Reiniciar", x + 50, y + 120, 120, 40, botao_reiniciar_estado)
    botao_continuar = desenhar_botao("Continuar", x + 230, y + 120, 120, 40,botao_continuar_estado)
    pygame.display.update()
    return botao_reiniciar, botao_continuar  # Retorna os retângulos dos botões para referência
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT :
                ...
                if event.key == pygame.K_LEFT:                      
                    mostrar_alerta(botao_reiniciar_estado = True,botao_continuar_estado = False)
                    botao_selecionado = 0
                elif event.key == pygame.K_RIGHT:                
                    mostrar_alerta(botao_reiniciar_estado = False,botao_continuar_estado = True)
                    botao_selecionado = 1
